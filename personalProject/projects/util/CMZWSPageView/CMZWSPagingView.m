...
@interface CMZWSPage (Private)

@property(nonatomic, assign) NSUInteger index;

@end

@implementation CMZWSPagingView
...
- (CMZWSPage *)pageAtLocation:(CGPoint)location {
    ...
    for (CMZWSPage *page in self.visiblePages) {
        if (page.index == index) {
            return page;
        }
    }
...
- (void)reloadPages {
    for (CMZWSPage *page in _visiblePages) {
        page.hidden = YES;
...
- (UIView *)dequeueReusablePage {
    CMZWSPage *page = [_recycledPages anyObject];
    if (page != nil) {
...
- (void)tilePages:(CGPoint)offset {
    ...
    for (NSNumber *number in indexes) {
        ...
        CMZWSPage *page = [_pagingDataSource pagingView:self pageForIndex:i % _numberOfPages];
        page.index = i;
