...
@interface CMZWSSectionBar ()

@end

@implementation CMZWSSectionBar
...
- (UIColor *)highlightedTextColor
{
    if (!_highlightedTextColor) {
        return [UIColor colorWithHexString:@"#ffaa00"] ;
    }
...
- (UIView *)itemForTitle:(NSString *)title {
    ...
    if (!item) {
        CMZWSMenuLabel *itemLabel = [CMZWSMenuLabel new];
...
- (void)resizeItem:(UIView *)item {
    if (CGSizeEqualToSize(self.itemSize, CGSizeZero)) {
        ...
        item.frame = CGRectOffset(item.frame, 15, 0);
    }
}
